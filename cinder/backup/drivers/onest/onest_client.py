...
#get location operate
    def get_location_without_bucket(self):
        if self.authinfo.access_net_mode == 1:
             resp = onest_common.GetLocationResponse(onest_common._make_request(self.authinfo,'GET', 'rest/', '', { 'intrasrvlocation': None}, {}))
             return resp.location
        elif self.authinfo.access_net_mode == 2: 
             resp = onest_common.GetLocationResponse(onest_common._make_request(self.authinfo,'GET', 'rest/', '', { 'intramgrlocation': None}, {}))
             return resp.location
        else :
             resp = onest_common.GetLocationResponse(onest_common._make_request(self.authinfo,'GET', 'rest/', '', {'outerlocation': None}, {}))
             return resp.location

    def get_location(self,bucket):
        if not self.bucketLocationMap.has_key(bucket):
            str = 'rest/%s'  % bucket
            if self.authinfo.access_net_mode == 1:
                  resp = onest_common.GetLocationResponse(onest_common._make_request(self.authinfo,'GET', str, '', { 'intrasrvlocation': None}, {}))
                  if resp.location == '':
                  ...
                  resp = onest_common.GetLocationResponse(onest_common._make_request(self.authinfo,'GET', str, '', { 'intramgrlocation': None}, {}))
                  if resp.location == '':
                      print 'get lvs location failed...'    
                      return ''
                  else :
                  ...
                  resp = onest_common.GetLocationResponse(onest_common._make_request(self.authinfo,'GET', str, '', {'outerlocation': None}, {}))
                  if resp.location == '':
                      print 'get lvs location failed...'
                      return ''
                  else :
...
#bucket operate
    def create_bucket(self, bucket):
        ...
        resp = onest_common.Response(onest_common._make_request(authinfo2,'PUT', bucket, '', {}, {}))
    ...
    def list_buckets_of_user(self):
        ...
        authinfo2 = self.resetLocation(self.location);
        return onest_common.ListBucketsOfUserResponse(onest_common._make_request(authinfo2,'GET'))
    ...
    def list_objects_of_bucket(self, bucket, options={}):
        ...
        if location =='':
            ...
            return onest_common.ListObjectsOfBucketResponse(onest_common._make_request(authinfo2,'GET', bucket, '', options))
    ...
    def delete_bucket(self, bucket):
        ...
        if location =='':
            ...
            authinfo2 = self.resetLocation(location);
            resp = onest_common.Response(onest_common._make_request(authinfo2,'DELETE', bucket, '', {}))
    ...
    def get_bucket_acl(self, bucket):
        ...
        if location =='':
            ...
            authinfo2 = self.resetLocation(location);
            return onest_common.GetAclResponse(onest_common._make_request(authinfo2,'GET', bucket, '', { 'acl': None }))
...
#object operate
    ...
    def get_object_meta(self, bucket, key):
        ...
        if location =='':
            ...
            authinfo2 = self.resetLocation(location);
            resp = onest_common.Response(
                    onest_common._make_request(authinfo2,'GET', bucket, key, {'meta': None}))
            if resp.http_response.status != 200:
                return None
            else :
    ...
    def get_object_data(self, bucket, key, headers={}):
        ...
        if location =='':
            ...
            resp = onest_common.GetResponse(
                    onest_common._make_request(authinfo2,'GET', bucket, key, {}, headers))
            if resp.http_response.status != 200:
                return None
            else :
                return resp.body
    ...
    def delete_object(self, bucket, key):
        ...
        if location =='':
            ...
            authinfo2 = self.resetLocation(location);
            resp = onest_common.Response(onest_common._make_request(authinfo2,'DELETE', bucket, key))
            if resp.http_response.status != 204:
                return False
            else :
                return True
    ...
    def delete_multi_object(self, bucket, delete_objects_xml_document):
        ...
        if location =='':
            ...
            authinfo2 = self.resetLocation(location);
            resp = onest_common.ListObjectsOfMultiDeletedResponse(onest_common._make_request(authinfo2, 'POST', bucket, '', { 'delete': None }, {}, delete_objects_xml_document))
            if resp.http_response.status != 200:
                return None
            else :
                return resp
    ...
    def get_object_acl(self, bucket, key):
        ...
        if location =='':
            ...
            return onest_common.GetAclResponse(
                onest_common._make_request(authinfo2,'GET', bucket, key, { 'acl': None }))
    ...
    def get_object_open_url(self, bucket, key, expireTime, bAnonymous):
        ...
        if bAnonymous == False :
            ...
            if resp.http_response.status != 200:
                ...
                ret1 += onest_common._gen_access_url_signature('GET', bucket, key, authinfo2.accessSecretkey,{'Date':s})
