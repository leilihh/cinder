...
class API(base.Base):
    ...
    def create(self, context, name, description, volume_id,
        ...
        if volume['status'] not in ['available', 'in-use']:
            msg = _('Volume to be backed up must be available or in-use')
            raise exception.InvalidVolume(reason=msg)

        # Online backup for 3par block storage
        is_online = False
        if volume['status'] in ['in-use']:
            LOG.info(_("############# LEILI: online create"))
            is_online = True
        ...
        LOG.info(_("############# LEILI: is_online = %d") % is_online)
