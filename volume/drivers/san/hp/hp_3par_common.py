...
class HP3PARCommon(object):
    ...
    def create_vlun(self, volume, host, nsp=None):
        ...
        is_online = volume.get('is_online', None)
        if is_online == True:
            LOG.debug(_('####################create_vlun: online mode'))
            volume_name = self._get_3par_snap_name(volume['snapshot_id'])
        else:
            LOG.debug(_('####################create_vlun: offline mode'))
            volume_name = self._get_3par_vol_name(volume['id'])
    ...
    def delete_vlun(self, volume, hostname):
        ...
        is_online = volume.get('is_online', None)
        if is_online == True:
            LOG.debug(_('###################delete_vlun: online mode'))
            volume_name = self._get_3par_snap_name(volume['snapshot_id'])
        else:
            volume_name = self._get_3par_vol_name(volume['id'])
    ...
    def get_cpg(self, volume, allowSnap=False):
        # 3par on_line support
        is_online = volume.get('is_online', None)
        if is_online == True:
            LOG.debug(_('#####################get_cpg: online mode')) 
            volume_name = self._get_3par_snap_name(volume['snapshot_id'])
        else:
            LOG.debug(_('#####################get_cpg: offline mode'))
            volume_name = self._get_3par_vol_name(volume['id'])
