...
class HP3PARFCDriver(cinder.volume.driver.FibreChannelDriver):
    ...
    def initialize_connection(self, volume, connector):
	    ...
	    print "====zouying=3par_fc: host=", host

            # now that we have a host, create the VLUN
            vlun = self.common.create_vlun(volume, host)


	    print "====zouying=3par_fc: vlun=", vlun

            target_wwns, init_targ_map = self._build_initiator_target_map(
                connector)


	    print "====zouying=3par_fc: target_wwns=", target_wwns
	    print "====zouying=3par_fc: init_targ_map=", init_targ_map
