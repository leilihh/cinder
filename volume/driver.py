...
class VolumeDriver(object):
    ...
    def backup_volume_online(self, context, backup, backup_service):
        """Create a new backup from an existing online volume"""
        volume = self.db.volume_get(context, backup['volume_id'])

        LOG.debug(_('Creating a new backup for online volume %s.') %
                  volume['name'])
        LOG.debug(_("################## LEILI: enter backup_volume_online"))
        # Struct a new snapshot metedata here passing to 3par driver
        options = {'volume_id': volume['id'],
                   ...
                   'display_name': "backup_snap",
                   'display_description': "snapshot for 3par backup",
                   'volume_type_id': volume['volume_type_id'],
                   'encryption_key_id': volume['encryption_key_id']}

        snapshot = self.db.snapshot_create(context, options)
        LOG.debug(_("################ LEILI: snapshot = %s"))
        # Create SnapShot which would attached to as a volume
        volume_snap = self.create_snapshot(snapshot)
        self.db.snapshot_update(context,
                                snapshot['id'], {'status': 'available',
                                                 'progress': '100%'})
        volume_snapshot = {'snapshot_id': snapshot['id'],
                           'is_online': True}

        properties = utils.brick_get_connector_properties()
        attach_info = self._attach_volume(context, volume_snapshot, properties)

        try:
            volume_snap_path = attach_info['device']['path']
            with utils.temporary_chown(volume_snap_path):
                with fileutils.file_open(volume_snap_path) as volume_file:
                    backup_service.backup(backup, volume_file)

        finally:
            self._detach_volume(context, attach_info, volume_snapshot, properties)
    ...
    def backup_volume(self, context, backup, backup_service):
        ...
        LOG.debug(_("############# LEILI: enter backup_volume"))
        properties = utils.brick_get_connector_properties()
        attach_info = self._attach_volume(context, volume, properties)
